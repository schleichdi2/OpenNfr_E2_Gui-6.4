		...
		if not dirs:
			for i in range(64):
				if (os.path.isfile("/usr/share/enigma2/spinner/wait%d.png"%(i+1))):
				        if not os.path.exists("/usr/share/enigma2/SpinnerSelector/lastused"):
                                                os.mkdir("/usr/share/enigma2/SpinnerSelector/lastused")
					shutil.copy("/usr/share/enigma2/spinner/wait%d.png"%(i+1), "/usr/share/enigma2/SpinnerSelector/lastused/wait%d.png"%(i+1))
					dirs = os.listdir(path)
                dirs.sort()
		menu = []
		for dir in dirs:
	...
	def menuCallback(self,choice):
		...
		for i in range(64):
			...
			if (os.path.isfile("/usr/share/enigma2/SpinnerSelector/%s/wait%d.png"%(choice,i+1))):
				shutil.copy("/usr/share/enigma2/SpinnerSelector/%s/wait%d.png"%(choice,i+1),  "/usr/share/enigma2/spinner/wait%d.png"%(i+1)) 
